# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: mhajji-b <mhajji-b@student.42.fr>          +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2019/10/11 21:19:28 by ncolomer          #+#    #+#              #
#    Updated: 2022/11/22 17:52:44 by mhajji-b         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

SRCS            =    ft_isalnum.c ft_isprint.c ft_memcmp.c ft_putchar_fd.c \
					ft_strncmp.c ft_atoi.c ft_isalpha.c  ft_strrchr.c \
					ft_memcpy.c ft_calloc.c ft_strlcpy.c ft_strchr.c \
					ft_tolower.c ft_bzero.c   ft_isascii.c ft_memcpy.c \
					ft_memmove.c  ft_strdup.c  ft_strlen.c \
					ft_toupper.c ft_isdigit.c ft_memchr.c  ft_memset.c \
					ft_strdup.c ft_memmove.c ft_strlcat.c ft_strnstr.c \
					ft_atoi.c ft_putstr_fd.c ft_putendl_fd.c ft_putnbr_fd.c \
					ft_strjoin.c ft_substr.c ft_split.c ft_strtrim.c \
					ft_itoa.c ft_strmapi.c ft_striteri.c
					 
OBJS            = $(SRCS:.c=.o)
OBJS_BNS        = $(BONUS:.c=.o)
BONUS           =  ft_lstadd_back.c ft_lstadd_front.c ft_lstclear.c \
                    ft_lstdelone.c  ft_lstiter.c ft_lstlast.c \
                    ft_lstnew.c ft_lstsize.c ft_lstmap.c 
CC                = gcc
RM                = rm -f
CFLAGS            = -Wall -Wextra -Werror

NAME            = libft.a

all:            $(NAME)

$(NAME):        $(OBJS)
				ar rcs $(NAME) $(OBJS)
%.o:%.c            
				$(CC) $(CFLAGS) -c $< -o $@
clean:
				$(RM) $(OBJS) $(OBJS_BNS)

fclean:            clean
				$(RM) $(NAME)

re:             fclean $(NAME)

bonus:          $(OBJS) $(OBJS_BNS)
				ar rcs $(NAME) $(OBJS) $(OBJS_BNS)

.PHONY:            all clean fclean re bonus